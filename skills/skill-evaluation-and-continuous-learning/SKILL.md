---
name: skill-evaluation-and-continuous-learning
description: Evaluate Skills and composed workflows with reproducible benchmarks, adversarial tests, metrics, regression and controlled updates.
metadata:
  version: "1.0"
...
---
## Purpose
Evaluate Skills and composed workflows with reproducible benchmarks, adversarial tests, metrics, regression and controlled updates.
...
Maintain golden tasks for every Skill and compound scenarios across Skills. Evaluate schema validity, instruction adherence, domain correctness, tool safety, end-to-end success, recovery and regression.

### Metrics
Track correctness, provenance/citation coverage, tool-call validity, unsafe-action rate, recovery rate, latency, cost, regression rate and calibration for probabilistic outputs.

### Update loop
`new evidence -> targeted test -> full regression -> version -> changelog -> monitor`.
...
Treat every material failure as a regression candidate; compare expected vs observed behavior, classify root cause, update the smallest responsible Skill, and re-run the suite.
