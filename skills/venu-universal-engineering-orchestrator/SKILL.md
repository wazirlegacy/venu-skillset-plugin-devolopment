---
name: venu-universal-engineering-orchestrator
description: Orchestrate the complete Venu engineering loop across Skills, models, research, tools, memory, validation and deployment.
metadata:
  version: "1.0"
...
---
## Purpose
Orchestrate the complete Venu engineering loop across Skills, models, research, tools, memory, validation and deployment.
...
Coordinate Skills, models, tools, memory, testing, security, deployment and feedback as explicit control planes.

### Master loop
`request -> assess -> research -> skill-route -> architecture -> plan -> authorize -> implement -> test -> secure -> observe -> deploy -> verify -> record -> learn`.

No consequential side effect without authorization, no production claim without verification, no persistent memory without provenance semantics, and no deployment without tests, health signals and rollback.
...
Treat the orchestrator as a control loop that routes Skills, models, retrieval, and tools while enforcing authorization, validation, observability, recovery, and memory.
