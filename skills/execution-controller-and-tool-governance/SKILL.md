...
description: Control tool execution with explicit permissions, side-effect classification, approvals, sandboxing, postconditions, auditability and rollback.
metadata:
  version: "1.0"
...
---
## Purpose
Control tool execution with explicit permissions, side-effect classification, approvals, sandboxing, postconditions, auditability and rollback.
...
The model proposes; deterministic policy authorizes; tools execute; validators verify.

### Lifecycle
`discover -> plan -> authorize -> prepare -> execute -> observe -> verify -> commit/rollback -> record`.

Classify actions as read-only, reversible write, external communication, destructive, or financial/high-consequence. Increase approval/isolation as risk rises. Tool contracts must declare schemas, scopes, limits, timeout, idempotency and rollback.
...
Place deterministic authorization, policy, schema validation, sandboxing, resource limits, approvals, and postcondition checks between reasoning and side effects.
