---
name: skill-router-and-composer
description: Select and compose the minimum sufficient Venu Skills for a task using intent, constraints, dependencies, risk, and required outputs.
metadata:
  version: "1.0"
...
---
## Purpose
Select and compose the minimum sufficient Venu Skills for a task using intent, constraints, dependencies, risk, and required outputs.
...
Select the minimum sufficient set of Venu Skills using task intent, domain entities, required artifacts, environment, dependencies, risk and expected information gain.

### Routing sequence
`classify -> extract constraints -> discover -> score -> dependency resolution -> conflict resolution -> compose -> hand off`.

Use exact domain matches first, then prerequisites and cross-cutting gates. Treat security/authorization/risk skills as hard gates for consequential actions. Record why each skill was selected, what input it consumes, what output it produces and what precedence it has.
...
Select minimum-sufficient Skills from task intent, dependencies, conflicts, precedence, risk, and expected outputs; compose explicit contracts rather than blending instructions blindly.
