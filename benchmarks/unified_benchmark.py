...
# Phase 1 portability fix: ROOT was Path(__file__).resolve().parent, i.e.
# the benchmarks/ folder itself, which has no SKILL.md files -- this script
# silently scanned zero Skills and reported a trivially "passing" empty
# result. Discovery/output paths now come from _skill_scope.py. See that
...
(OUTPUT_DIR/'unified_benchmark_results.json').write_text(json.dumps({'summary':summary,'results':rows},indent=2))
